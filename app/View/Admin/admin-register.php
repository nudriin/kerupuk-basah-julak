<section x-data="{open : true}">
    <div class="container items-center justify-center px-4 py-4 mx-auto mt-12 lg:px-20 lg:py-28 lg:mt-0">
        ...
        <?php } ?>
        <div class="p-4">
            <h1 class="mb-5 text-4xl font-medium text-center font-rubik">Tambah admin</h1>
            <div class="w-full mx-auto bg-white rounded-lg shadow-xl lg:w-9/12">
                <form action="/admin/register" method="post" class="flex flex-col gap-3">
                    <div class="flex flex-col gap-1 px-4 mt-4">
                        ...
                        <input type="text" name="username" class="p-2 border-2 rounded-lg border-abu" value="<?= $model['post_username'] ?? "" ?>">
                    </div>
                    <div class="flex flex-col gap-1 px-4 mt-4">
                        <label for="" class="text-xl font-semibold">Email</label>
                        <input type="email" name="email" class="p-2 border-2 rounded-lg border-abu" value="<?= $model['post_email'] ?? "" ?>">
                    </div>
                    <div class="flex flex-col gap-1 px-4">
                        <label for="" class="text-xl font-semibold">Nama</label>
                        <input type="text" name="name" class="p-2 border-2 rounded-lg border-abu" value="<?= $model['post_name'] ?? "" ?>">
                    </div>
                    <div class="flex flex-col gap-1 px-4">
                        <label for="" class="text-xl font-semibold">Nomor HP</label>
                        <input type="number" name="phone" class="p-2 border-2 rounded-lg border-abu" value="<?= $model['post_phone'] ?? "" ?>">
                    </div>
                    <div class="flex flex-col gap-1 px-4">
                        <label for="" class="text-xl font-semibold">Password</label>
                        <input type="password" name="password" class="p-2 border-2 rounded-lg border-abu">
                    </div>
                    <div class="flex items-center justify-center px-4 mt-4 mb-4">
                        <button type="submit" class="w-full p-2 text-xl font-semibold text-center rounded-lg text-dark-white bg-merah">Daftar</button>
                    </div>
                </form>
            </div>
        </div>
